...
public class AsientoService {
    ...
    private static Map<String, Espectador> mapaAsientos = new TreeMap(AsientoService.ordenar);
    //SETTER
    public static Map<String, Espectador> getMapaAsientos() {  //forzar static
        return mapaAsientos;
    }
    //GETTER
    public static void setMapaAsientos(Map<String, Espectador> mapaAsientos) { //forzar static
        AsientoService.mapaAsientos = mapaAsientos;
    }
    //-----------------------------------------------    creo el comparator de la misma
    ...
    public static Map<String, Espectador> crearSala(){
        ...
        for (int i=0; i<filas.length; i++) {
            Asiento asientoA = new Asiento();
            
            asientoA.setNumero(n);                
            asientoA.setOcupado(false);
            asientoA.setEspectador(null);
            asientoA.setLetra(filas[i]);
            
            for (int j=0; j<filas.length; j++) {
                Asiento asientoB = new Asiento();
                
                asientoA.setLetra(filas[j]);
                asientoB.setNumero(asientoA.getNumero());
                asientoB.setOcupado(asientoA.isOcupado());  //se corrige solo
                asientoB.setEspectador(asientoA.getEspectador());
                
                String llave = asientoA.getNumero() + asientoA.getLetra();
                
                mapaAsientos.put(llave, null);  //NOTA: debe ser 'private static' la lista
            }
        ...
        System.out.println("\033[35m"+"Tamaño de sala -> "+mapaAsientos.size()+" butacas");
